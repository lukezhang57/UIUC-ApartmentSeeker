...
class DistanceMatrix(models.Model): 
    ...
    apartment =  models.ForeignKey(Apartment, on_delete=models.CASCADE,null=True) 
    direct_distance = models.DecimalField(decimal_places = 2, default = 0, max_digits=10, null=True)
    ...
    def save(self,  *args, **kwargs):
        if self.important_building and self.apartment:
            self.direct_distance = float(self.important_building.address.dist(self.apartment.address))
